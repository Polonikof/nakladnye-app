...
ARCHIVE_DIR = "_архив"
INSTRUCTION_NAME = "КАК_КЛАСТЬ_ФАЙЛЫ.txt"

SOURCE_MARKERS = ("source", "исход", "сырой", "вход")
READY_MARKERS = ("готов", "result", "эталон", "образец")
EXCEL_EXT = (".xls", ".xlsx")

INSTRUCTION = """Как обучить программу новому формату

1. Положите СЮДА два файла одного поставщика / одной накладной:
     • исходник  — как пришло (в имени можно написать «source» или «исходник»)
     • готовый   — как вы обработали руками (в имени «готовый»)

   Пример:
     ВВП УПД 04.09.26 source.xlsx
     ВВП УПД 04.09.26 готовый.xlsx

   Можно положить пару в подпапку:
     НовыеНакладные\\Обои_май\\исходник.xlsx
     НовыеНакладные\\Обои_май\\готовый.xlsx

2. Запустите обучение одним из способов:
     • в окне Накладные.exe нажмите «Обучить из НовыеНакладные»
     • Следить_за_НовыеНакладные.bat  (сам увидит новые файлы)
     • python learn_novye.py --once

3. Агент разберёт пару, допишет артикулы в справочник oboi_catalog.json
   и положит отчёт в подпапку _архив.

4. Справочник рядом с Накладные.exe подхватывается без пересборки.
   Чтобы вшить его в новый .exe — запустите build_exe.bat
   (это и есть новый релиз).
"""
